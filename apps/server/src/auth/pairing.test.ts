...
describe('authenticateDevice', () => {
  ...
  it('rejects a revoked device', async () => {
    const { response } = await pairDevice()
    await revokeDevice(db, response.deviceId)
...
describe('device management', () => {
  it('lists paired devices newest first', async () => {
  ...
  it('revokes one device without affecting the others', async () => {
    ...
    await revokeDevice(db, first.response.deviceId)

    expect(await authenticateDevice(db, first.response.deviceToken)).toBeNull()
    expect(await authenticateDevice(db, second.response.deviceToken)).not.toBeNull()
  })
  ...
  it('reports revoking twice as no change the second time', async () => {
    ...
    expect(await revokeDevice(db, response.deviceId)).toBe(true)
    expect(await revokeDevice(db, response.deviceId)).toBe(false)
  })
})
